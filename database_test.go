...
func setupDB(t *testing.T) *trackDB {
	...
	//Unable to get connection for unknown reason
	//This course is so bad at learnin us how to do stuff
	db := trackDB{
		"mongodb://localhost",
		"testtrackdb",
		"tracks",
...
func TestTrackDB_Add(t *testing.T) {
	...
	db.Init()
	if db.Count() != 0 {
		t.Error("database not properlu initialized. track count() should be 0")
	}
